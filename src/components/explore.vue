...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
      for (let i = 0; i < 4; i++) {
        this.items.push('more' + ++this.itemIndex + 'more')
      }
    },
    watch: {
      scrollbarObj: {
    ...
    methods: {
      ...
      onPullingDown () {
        ...
        setTimeout(() => {
          ...
          if (Math.random() > 0.5) {
            // 如果有新数据
            this.items.unshift('more' + +new Date())
          } else {
            // 如果没有新数据
      ...
      onPullingUp () {
        ...
        setTimeout(() => {
          ...
          if (Math.random() > 0.5) {
            ...
            let newPage = []
            for (let i = 0; i < 10; i++) {
              newPage.push('more' + ++this.itemIndex + 'more')
            }
